...
#define MIN_SIZE 3.0f
	...
	private:
		...
		void GenInitLocation(Parti partciles[], int nums);//生成初始粒子

		unsigned int mCurVBO, mCurTransformFeedbackIndex;
		GLuint ParticleBuffers[2]; //粒子发射系统的两个顶点缓存区
		GLuint ParticleArrays[2];
		GLuint TransformFeedbacks[2];//粒子发射系统对应的TransformFeedback
		GLuint RandomTexture;//随机一维纹理
		CTexture floTexture;//Alpha纹理
